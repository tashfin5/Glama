  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setIsKBeauty(p.isKBeauty !== undefined ? p.isKBeauty : true);
        setTags(p.tags ? p.tags.join(', ') : '');
        setImages(p.images || []);
        
        const catId = typeof p.category === 'object' && p.category !== null ? p.category._id : p.category;
        setCategory(catId || (categoriesRes.data.length > 0 ? categoriesRes.data[0]._id : ''));
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-serif font-medium tracking-wide text-gray-900 uppercase">Edit Product</h1>
          </div>
        </div>
      </div>
